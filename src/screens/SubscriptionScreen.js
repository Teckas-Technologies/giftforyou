...
import { getOfferings, purchasePackage, restorePurchases, openManageSubscriptions } from '../services/billing';
import { CustomAlert } from '../components';
import useAlert from '../hooks/useAlert';
import { colors } from '../theme';

const BackIcon = ({ size = 24, color = colors.textLight }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
    <Polyline points="15 18 9 12 15 6" />
  </Svg>
);

const GiftIcon = ({ size = 26, color = '#FFFFFF' }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
...
const StarIcon = ({ size = 26, color = '#FFFFFF' }) => (
...
const BuildingIcon = ({ size = 26, color = '#FFFFFF' }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
...
const CheckIcon = ({ size = 16, color = colors.secondary }) => (
...
const PLAN_COPY = {
  free: { title: "You're on the Free Plan", subtitle: 'Subscribe to unlock everything Thoughtfully has to offer.', Icon: GiftIcon },
  individual: { title: "You're on the Individual Plan", subtitle: 'Thanks for supporting Thoughtfully! 💕', Icon: StarIcon },
  organization: { title: "You're on the Organization Plan", subtitle: 'Access provided by your company.', Icon: BuildingIcon },
};
...
const SubscriptionScreen = ({ navigation }) => {
  ...
  const sortedOfferings = useMemo(() => {
    const rank = (pkg) => (pkg.packageType === 'ANNUAL' ? 1 : 0);
  ...
  const yearlySavingsPercent = monthly && yearly
  ...
  const selectedPackage = sortedOfferings.find((p) => p.identifier === selectedId) || sortedOfferings[0];
  ...
  const statusSubtitle = plan === 'organization' && organizationName
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                ...
                <>
                  <View style={styles.planOptionsRow}>
                    {sortedOfferings.map((pkg) => {
                      ...
                      const periodLabel = isYearly ? 'Yearly' : pkg.packageType === 'MONTHLY' ? 'Monthly' : pkg.product.title;
                      const periodSuffix = isYearly ? '/ year' : pkg.packageType === 'MONTHLY' ? '/ month' : '';
                      return (
                        <TouchableOpacity
                          key={pkg.identifier}
                        ...
                        >
                          {isYearly && yearlySavingsPercent > 0 && (
                            <View style={styles.savingsBadge}>
                              <Text style={styles.savingsBadgeText}>SAVE {yearlySavingsPercent}%</Text>
                            </View>
                          )}
                          <Text style={[styles.planOptionLabel, isSelected && styles.planOptionLabelSelected]}>
                            {periodLabel}
                          </Text>
                          <Text style={[styles.planOptionPrice, isSelected && styles.planOptionPriceSelected]}>
                            {pkg.product.priceString}
                          </Text>
                          {!!periodSuffix && (
                            <Text style={[styles.planOptionSuffix, isSelected && styles.planOptionSuffixSelected]}>
                              {periodSuffix}
                            </Text>
                          )}
                        </TouchableOpacity>
                    ...
                    >
                      {purchasingId ? (
                        ...
                        <Text style={styles.subscribeButtonText}>
                          Subscribe{selectedPackage ? ` — ${selectedPackage.product.priceString}` : ''}
                        </Text>
                      )}
                    </LinearGradient>
                  </TouchableOpacity>
          ...
          {plan === 'free' && (
            <>
              <TouchableOpacity onPress={handleRestore} disabled={restoring} style={styles.footerLink}>
                <Text style={styles.footerLinkText}>
                  {restoring ? 'Checking your account...' : 'Already subscribed? Restore it here'}
                </Text>
              </TouchableOpacity>

              <TouchableOpacity onPress={() => navigation.navigate('RedeemCoupon')} style={styles.footerLink}>
                <Text style={styles.footerLinkText}>
