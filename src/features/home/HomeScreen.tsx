...
// Users Icon SVG
const UsersIcon = ({ size = 18, color = '#ca9ad6' }: IconProps) => (
  <Svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
// Calendar Icon SVG
const CalendarIcon = ({ size = 18, color = '#ca9ad6' }: IconProps) => (
  <Svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
// Gift Icon SVG
const GiftIcon = ({ size = 18, color = '#ca9ad6' }: IconProps) => (
  <Svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const CakeIcon = ({ size = 18, color = '#ca9ad6' }: IconProps) => (
  <Svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
// Sparkle Icon
const SparkleIcon = ({ size = 16, color = '#70d0dd' }: IconProps) => (
...
const getDaysUntil = (dateStr?: string) => appDaysUntil(dateStr) ?? 0;

// Helper to get badge info
const getBadgeInfo = (daysUntil: number) => {
...
const HomeScreen = ({ navigation }: ScreenProps) => {
  ...
  const createFadeStyle = (anim: Animated.Value | Animated.AnimatedInterpolation<number>) => ({
    opacity: anim,
    transform: [
      {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        >
          ...
          >
            ...
            >
              <Text style={[styles.greetingMask, { opacity: 0 }]}>
                {getGreeting()}, {userName || 'there'}!
              </Text>
            </LinearGradient>
          </MaskedView>
          ...
          {[
            {
              ...
              label: 'Contacts',
              colors: ['#70d0dd', '#99e5ed', '#ccf9ff'] as const,
            ...
            {
              icon: CakeIcon,
              value: String(stats.birthdaysThisMonth),
              label: 'Birthdays',
              colors: ['#70d0dd', '#99e5ed', '#ccf9ff'] as const,
            },
          ].map((stat, index) => (
        ...
        <Animated.View style={[styles.sectionHeader, createFadeStyle(sectionAnim)]}>
          ...
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('Calendar')}
          >
            <MaskedView maskElement={<Text style={styles.seeAllMask}>See all →</Text>}>
              <LinearGradient
                ...
                >
                  ...
                  >
                    ...
                    <View style={styles.birthdayInfo}>
                      ...
                      <Text style={styles.birthdayRelation} numberOfLines={1}>
                        {event.event_type || event.eventType || 'Event'}
                      </Text>
                    </View>
                    <View style={styles.birthdayDateInfo}>
                      <Text style={styles.birthdayDate}>
                        {formatEventDate(event.event_date || event.eventDate)}
                      </Text>
        ...
        {!loading && upcomingEvents.length > 1 && (
          <View style={styles.pagerDots}>
            {upcomingEvents.slice(0, 5).map((_, i) => (
              <View
                key={i}
                style={[styles.pagerDot, i === activeEventIndex && styles.pagerDotActive]}
              />
            ))}
          </View>
        )}
        ...
        <Animated.View style={[styles.quickSection, createFadeStyle(quickAnim)]}>
          ...
          <View style={styles.quickRow}>
            <TouchableOpacity
              style={styles.quickBtnContainer}
              activeOpacity={0.8}
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
            onPress={() => navigation.navigate('SendLoveNote')}
          >
      ...
      <LoveNotePopup visible={!!loveNote} text={loveNote?.text} onClose={() => setLoveNote(null)} />
    </View>
  );
};
