...
const UsersIcon = ({ size = 24, color = '#6b3a8a' }: IconProps) => (
  <Svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const XIcon = ({ size = 20, color = '#6b3a8a' }: IconProps) => (
  <Svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const CheckIcon = ({ size = 18, color = '#FFFFFF' }: IconProps) => (
  <Svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const SearchIcon = ({ size = 20, color = '#6b3a8a' }: IconProps) => (
  <Svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const getMutualText = (count?: number) => {
...
const DiscoverScreen = ({ navigation }: ScreenProps) => {
  ...
  const [pipeline, setPipeline] = useState<Record<string, any>>({}); // { [userId]: { id, name, ..., status } }
  ...
  // sees the upgrade prompt the instant they tap Connect — no spinner, no
  ...
  const buildSuggestions = (rawSuggestions: any[]) =>
    ...
    try {
      ...
      const acceptedIds = new Set(
        contacts
          ...
          .map((c: any) => c.member?.id || c.member_id || c.memberId)
          .filter(Boolean),
      );
      ...
      if (searchResp) {
        ...
        setSearchResults(users.filter((u: any) => !acceptedIds.has(u.id)));
      }
    } catch (error) {
  ...
  const renderSuggestionCard = (user: any, index: number) => {
    const userId = user._id || user.id;
    ...
    return (
      <View key={userId} style={styles.card}>
        ...
        <TouchableOpacity
          ...
          // again.
          onPress={() =>
            (!user.status || user.status === 'declined_acknowledged') && handleAddToCircle(user)
          }
          disabled={isAdding || (!!user.status && user.status !== 'declined_acknowledged')}
          activeOpacity={!user.status || user.status === 'declined_acknowledged' ? 0.7 : 1}
        >
          <LinearGradient
            colors={
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <MaskedView maskElement={<Text style={styles.headerTitleMask}>Discover</Text>}>
            <LinearGradient
              colors={['#ca9ad6', '#70d0dd']}
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 1 }}
  ...
  const displayedSuggestions = [...suggestions.filter((s) => !pipeline[s.id]), ...pipelineCards];
              ...
              <>
                ...
                <View style={styles.sectionHeader}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.sectionSubtitle}>Friends of your friends</Text>
                  </View>
                </View>
