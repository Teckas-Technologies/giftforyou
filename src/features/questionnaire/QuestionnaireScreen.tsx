...
const usToISO = (v: any) => {
  ...
  return m ? `${m[3]}-${m[1].padStart(2, '0')}-${m[2].padStart(2, '0')}` : s; // partial typing or already ISO — leave as-is
};
...
const CheckIcon = ({ size = 20, color = '#FFFFFF' }: IconProps) => (
  <Svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
    ...
    questions: [
      {
        ...
        options: [
          ...
          { id: 'tulip', label: 'Tulip', emoji: '🌷' },
          {
            id: 'lavender',
            label: 'Lavender',
            image: require('../../../assets/emoji-lavender.png'),
          },
        ...
        options: [
          {
            id: 'chocolate',
            label: 'Chocolate',
            image: require('../../../assets/emoji-chocolate.png'),
          },
    ...
    questions: [
      {
        ...
        placeholder:
          "List items you'd love to receive...\n\n• New yoga mat\n• Wireless earbuds\n• Cooking class gift card",
      },
      {
        id: 'wishlist_link_1',
        type: 'url',
...
const QuestionnaireScreen = ({ navigation, route }: ScreenProps) => {
    ...
    try {
      ...
      if (response.questionnaire) {
        ...
        Object.keys(mappedAnswers).forEach((key) => {
          const val = (mappedAnswers as any)[key];
          if (val === '' || (Array.isArray(val) && val.length === 0)) {
            delete (mappedAnswers as any)[key];
          }
        });
      ...
      setAnswers((prev: any) => ({
        ...
        ...(anc[0]?.date
          ? { anniversary_1: anc[0].date, anniversary_1_title: anc[0].title || '' }
          : {}),
        ...(anc[1]?.date
          ? { anniversary_2: anc[1].date, anniversary_2_title: anc[1].title || '' }
          : {}),
        ...(anc[2]?.date
          ? { anniversary_3: anc[2].date, anniversary_3_title: anc[2].title || '' }
          : {}),
      }));
    } catch (error) {
  ...
  const handleTextChange = (questionId: string, text: string) => {
    setAnswers((prev: any) => ({ ...prev, [questionId]: text }));
  };

  const isAnswerEmpty = (question: any) => {
  ...
  const getPlaceholder = (question: any) => {
    ...
    const phrases = selected.map((optionId) => dp.examples[optionId]).filter(Boolean);
  ...
  const validateCurrentSection = () => {
    const missing = section.questions.filter((q: any) => q.required && isAnswerEmpty(q));
    if (missing.length === 0) return true;
    const labels = missing.map((q: any) => `• ${q.question}`).join('\n');
    // A missing field isn't an "error" — nothing went wrong, they just
    // haven't finished yet. A warning-styled nudge reads much friendlier
    // than a red error icon for something this routine.
    showWarning(
  ...
  const optionMatchesValue = (option: any, value: any) =>
    value != null && (norm(value) === norm(option.id) || norm(value) === norm(option.label));
  ...
  const asArray = (v: any) => (Array.isArray(v) ? v : v == null || v === '' ? [] : [v]);

  const renderOption = (option: any, questionId: string, type: string) => {
    const isSelected =
      type === 'multiselect'
        ? asArray(answers[questionId]).some((v: any) => optionMatchesValue(option, v))
  ...
  const QuestionLabel = ({ question }: { question: any }) => (
  ...
  const renderQuestion = (question: any) => {
    ...
    const allSelected =
      ...
      question.options.every((opt: any) =>
        asArray(answers[question.id]).some((v: any) => optionMatchesValue(opt, v)),
      );

    const handleSelectAll = () => {
      if (allSelected) {
        setAnswers((prev: any) => ({ ...prev, [question.id]: [] }));
      } else {
        setAnswers((prev: any) => ({
          ...prev,
          [question.id]: question.options.map((opt: any) => opt.id),
        }));
      }
    };
    ...
    return (
      <View key={question.id} style={styles.questionContainer}>
        ...
        <View style={styles.optionsGrid}>
          {question.options.map((option: any) => renderOption(option, question.id, question.type))}
        </View>
      </View>
    );
  };
          ...
          {!loading && (
            ...
            >
              ...
              <View style={styles.sectionHeader}>
                ...
                <MaskedView
                  maskElement={<Text style={styles.sectionTitleMask}>{section.title}</Text>}
                >
                  <LinearGradient
