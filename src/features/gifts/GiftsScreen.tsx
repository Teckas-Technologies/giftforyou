...
import type { IconProps } from '../../types/navigation';
...
const GiftsScreen = () => {
  ...
  const createSlideStyle = (anim: Animated.Value, index: number) => ({
    opacity: anim,
    transform: [
      {
  ...
  const getEmojiBackground = (colorType: string) => {
  ...
  const renderGiftCard = (gift: any, index: number) => (
    ...
    >
      <TouchableOpacity style={styles.giftCardInner} activeOpacity={0.8}>
        ...
        >
          <LinearGradient
            colors={gift.colorType === 'pink' ? ['#fbe5f5', '#f4cae8'] : ['#ccf9ff', '#a8e6f0']}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 1 }}
        ...
        >
          <TouchableOpacity>
            <HeartIcon size={24} filled={gift.saved} color={gift.saved ? '#ca9ad6' : '#d0d0d0'} />
          </TouchableOpacity>
        </Animated.View>
      </TouchableOpacity>
    </Animated.View>
    ...
    <View style={styles.container}>
      ...
      <Animated.View style={[styles.header, createFadeStyle(headerAnim)]}>
        <View style={styles.headerLeft}>
          <MaskedView maskElement={<Text style={styles.headerTitleMask}>Gift Ideas</Text>}>
            <LinearGradient
        ...
        <Animated.View
          style={{
            transform: [{ translateY: floatTranslateY }, { scale: pulseAnim }],
          }}
        >
