...
// Search Icon SVG
const SearchIcon = ({ size = 18, color = '#6b3a8a' }: IconProps) => (
  <Svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
// Calendar Icon SVG
const CalendarIcon = ({ size = 12, color = '#6b3a8a' }: IconProps) => (
...
// Plus Icon SVG
const PlusIcon = ({ size = 24, color = 'white' }: IconProps) => (
  <Svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const UsersIcon = ({ size = 22, color = '#ca9ad6' }: IconProps) => (
  <Svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
// Chevron Right Icon
const ChevronRightIcon = ({ size = 18, color = '#ca9ad6' }: IconProps) => (
  <Svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const getAvatarStyle = (colorType: string) => {
  if (colorType === 'pink') {
    return { bg: '#fbe5f5', color: '#ca9ad6' };
  }
  return { bg: '#ccf9ff', color: '#70d0dd' };
...
const ContactsScreen = ({ navigation }: ScreenProps) => {
  ...
  const createFadeStyle = (anim: Animated.Value) => ({
    opacity: anim,
    transform: [
      {
  ...
  const tabScopedContacts =
    activeTab === 'all' ? contacts : contacts.filter((c) => c.relationTag === activeTab);
  ...
  return (
    ...
    >
      ...
      <Animated.View style={[styles.header, createFadeStyle(headerAnim)]}>
        <MaskedView maskElement={<Text style={styles.headerTitleMask}>Contacts</Text>}>
          <LinearGradient
