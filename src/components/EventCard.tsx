...
type EventType = 'birthday' | 'wedding' | 'babyShower' | 'custom';
...
const EventCard = ({
  ...
  const getEventIcon = (): { name: IoniconName; color: string } => {
    switch (eventType) {
      case 'wedding':
    ...
    <TouchableOpacity style={[styles.container, style]} onPress={onPress} activeOpacity={0.9}>
      <View style={styles.leftSection}>
        <View style={styles.avatarContainer}>
          ...
          <View style={[styles.eventIconBadge, { backgroundColor: eventIcon.color }]}>
      ...
      <View style={styles.rightSection}>
        <Badge text={formatCountdown(daysUntil)} variant={getBadgeVariant(daysUntil)} />
      </View>
    </TouchableOpacity>
  );
};
